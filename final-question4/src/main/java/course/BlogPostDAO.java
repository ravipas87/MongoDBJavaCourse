...
public class BlogPostDAO {
    ...
    public void likePost(final String permalink, final int ordinal) {
    	...
    	//find by permalink
    //	List<Document> comments =(List<Document>) findByPermalink(permalink).get("comments");
    //	int num_likes= comments.get(ordinal).getInteger("num_likes");
    	
    	
    	BasicDBObject incrementedLikes =
    			new BasicDBObject().append("$inc",
    			new BasicDBObject().append("comments."+String.valueOf(ordinal)+".num_likes", 1));

    	postsCollection.updateOne(eq("permalink", permalink), incrementedLikes);
    	//fin ravipas
    }
}
